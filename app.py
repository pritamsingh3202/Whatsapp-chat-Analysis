...
#     fetch unique users
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        with col1:
            ...
            busy_date = helper.week_activity_map(selected_user, df)
            fig, ax = plt.subplots()
            plt.xticks(rotation= 'vertical')
            ax.bar(busy_date.index, busy_date.values)
            st.pyplot(fig)
        ...
        if selected_user == 'Overall Users':
            st.title('Most Busy Users In Group')
            x, new_df = helper.most_buys_user(df)
            fig, ax = plt.subplots()
            col1, col2 = st.columns(2)
...
#       emoji analysis
        ...
        emoji_df = helper.emojis_finder(selected_user, df)
